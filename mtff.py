...
class MTFF:
    ...
    def default_args(cls):
        ...
        # 数据集相关
        parser.add_argument('--slice_edge_type', type=str, default="[0,1,2, 3]", help='数据集中边的数量。')
    ...
    def save_model(self, path):
        """
        It saves the model, optimizer and the parameters.
        ...
        if self.args.load_model_file is not None:
            ...
            for files in os.listdir(self.args.feature_model_dir):
                if files.endswith("joblib"):
                    ...
                    self.feature_models.append(temp_model)
            
            
            self.model = name_to_model(self.args.backbone_model, self.args, feature_models=self.feature_models)
    ...
    def __run_epoch(
        ...
        for batch_data in data.batch_generator():
            ...
            if data_fold == DataFold.VALIDATION:
                # eval 修改batchnorm等。 no_grad设置没有梯度传播。
                self.model.eval()
