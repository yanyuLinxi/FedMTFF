...
BIG_NUMBER = 1e7
class PYGraphDataNaming(Data):
    def __init__(self):
        super(PYGraphDataNaming, self).__init__()
    """
    def __init__(self, edge_index, edge_index_three_hop, edge_index_five_hop, label, x):
        ...
        self.x = x
    """
    def load_attr(self, edge_index, edge_index_two_hop, edge_index_three_hop, edge_index_five_hop, edge_index_ncs, edge_index_dfg, edge_index_last_write, edge_index_last_use, edge_index_self_loop, value_label, type_label, label, x, x_type, right_most, candidate_id, candidate_id_mask):
        ...
        """
        self.edge_index = edge_index
        self.edge_index_two_hop = edge_index_two_hop
        self.edge_index_three_hop = edge_index_three_hop
        self.edge_index_five_hop = edge_index_five_hop
...
def PYNamingMake_task_input(batch_data, get_num_edge_types=False):
...
class PYGraphDatasetNaming(Dataset):
    def __init__(self, root, value_dict, type_dict, graph_node_max_num_chars=19, max_node_per_graph=50,max_graph=20000, max_variable_candidates=5, transform=None, pre_transform=None):
        ...
        super(PYGraphDatasetNaming, self).__init__(root, transform, pre_transform)
    ...
    def processed_dir(self):
        ...
        return osp.join(self.root, "processed_naming")
    ...
    def process(self):
        ...
        # 添加了 last use last write边。但是这里我们每个边里都包括self loop边。
        # 最终使用边的时候不包括self loop edge。但是每个边里有self-loop。
        data_i = 0
        ...
        for raw_path in self.raw_paths:
            with open(raw_path, "r") as f:
                ...
                for line in lines:
                    ...
                    if not astEdges or not three_neighbors_adj or not two_neighbors_adj or not five_neighbors_adj or not dfg_edge_index or not ncs_edge_index:
                        continue
                    ...
                    for j in range(50, num_nodes):
                        ...
                        candidate_node_ids = [type_ane_value_correct_id_list[0]] + other_id_list[:self.max_variable_candidates-1] 
                        candidate_node_ids_mask = [True] * self.max_variable_candidates
                        

                        data = PYGraphDataNaming()
                        data.load_attr(
