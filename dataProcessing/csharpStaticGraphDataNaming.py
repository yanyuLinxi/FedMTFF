...
from torch_geometric.utils import to_undirected, add_self_loops, subgraph, is_undirected, remove_self_loops
...
class CSharpStaticGraphData(Data):
    def __init__(self):
        super(CSharpStaticGraphData, self).__init__()
...
def CSharpStaticNamingMake_task_input(batch_data, get_num_edge_types=False):
...
class CSharpStaticGraphDatasetNaming(Dataset):
    ...
    def __init__(self,
        ...
        super(CSharpStaticGraphDatasetNaming, self).__init__(root, transform, pre_transform)
    ...
    def trans_list_to_edge_tensor(self,
        ...
        if not is_add_self_loop:
            t, attr = remove_self_loops(t)
        ...
        for raw_path in self.raw_paths:
            with gzip.open(raw_path, "r") as f:  # josnl.gz文件先通过gz读取，再按行读取。
                ...
                for raw_dict in raw_list:
                    ...
                    ## 解决naming的slot id 问题和value label问题。
                    tgt_node_id_str = str(self.max_node_per_graph)
                    if tgt_node_id_str not in NodeTypes:
                        # 当不存在节点类型，或者不存在节点label "EMPTY"
                        value_label = 0  
                    elif tgt_node_id_str not in NodeLabels:
                        continue
                    else:
                        value_label = int(self.value_dict.get(NodeLabels[tgt_node_id_str], len(self.value_dict)))
                    SlotDummyNode = self.max_node_per_graph-1
                    
                    #type_label = int(self.type_dict.get(NodeTypes[str(self.max_node_per_graph)], len(self.type_dict)))
                    type_label = 0
...
if __name__ == '__main__':
    ...
    gd = CSharpStaticGraphDatasetNaming("data/lrtemp/",
